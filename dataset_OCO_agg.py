...
def get_500m_dataset(xLims=np.array([10,15]), yLims=np.array([45,50]), xLims_test = np.array([10, 15]), yLims_test = np.array([45, 50]), d_start=0, d_end=46, USE_MODEL_OUT=True,SCALE_FACTOR=10,DATA_OPTION=1):
    ...
    pathLoadTopography = pathPref + 'Topography_0005_E-10_50_N25_70_USGS_GMTED2010.nc'
    ...
    topography = nc_to.variables['topography'][iy_min_lm:iy_max_lm,ix_min_lm:ix_max_lm].data
    ...
    if DATA_OPTION == 1:
        ...
        for N_oco in range(len(df_oco)):
            ...
            Masked_dcCorr_feat = dcCorr_feat[step, y_min:y_max, x_min:x_max]
            ...
            Masked_green = green[step,y_min:y_max, x_min:x_max]
            Masked_swir1 = swir1[step,y_min:y_max, x_min:x_max]
            Masked_swir2 = swir2[step,y_min:y_max, x_min:x_max]
            Masked_swir3 = swir3[step,y_min:y_max, x_min:x_max]
            Masked_nirv = nirv[step,y_min:y_max, x_min:x_max]
            Masked_kNDVI = kNDVI[step,y_min:y_max, x_min:x_max]
            Masked_evi = evi[step,y_min:y_max, x_min:x_max]
            Masked_ndvi = ndvi[step,y_min:y_max, x_min:x_max]
            Masked_cosSZA = cosSZA[step,y_min:y_max, x_min:x_max]
            Masked_temperature = temperature[step,y_min:y_max, x_min:x_max]
            Masked_precipitation = precipitation[step,y_min:y_max, x_min:x_max]
            Masked_temperature_del = temperature_del[step,y_min:y_max, x_min:x_max]
            Masked_precipitation_del = precipitation_del[step,y_min:y_max, x_min:x_max]
