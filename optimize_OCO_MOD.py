...
import pytorch_ssim
import helpers
from dataset_OCO_agg import get_500m_dataset
from dataset_OCO_agg import DataSet
from ModelClass5_5 import SR_SIF
...
results_folder = 'DO'+str(DATA_OPTION)+'_MOD0'+str(MOD) + '.5_comp2OCO_DLcorrected_train_E5-20_N40-55_test_E10-15_N45-50_small_MOD/'
...
if USE_BEST_PARAMS:
  ...
  #best_params = {'learning_rate': 9.655093202037676e-07, 'weight_decay': 0.06890580184915472, 'max_epochs': 16} #best par of the new model
  #best_params = {'learning_rate': 1.655093202037676e-08, 'weight_decay': 0.06890580184915472, 'max_epochs': 11} #best par of the new model
  #best_params = {'learning_rate': 9.476595438205447e-03, 'weight_decay': 0.09397165477798103, 'max_epochs': 9} #best par of the new model
  best_params = {'learning_rate': 9.95018692735643e-05, 'weight_decay': 0.00044042494966214577, 'max_epochs': 19}#38
...
def Calc_Loss(pred, target):
  ...
  if torch.nanmean(target).float() > 1:
    loss = mse_loss(torch.nanmean(pred).float(), torch.nanmean(target).float())*100
  elif torch.nanmean(target).float() > 0.7:
    loss = mse_loss(torch.nanmean(pred).float(), torch.nanmean(target).float())*50 
  elif torch.nanmean(target).float() > 0.4:
    loss = mse_loss(torch.nanmean(pred).float(), torch.nanmean(target).float())*25
  else:
    loss = mse_loss(torch.nanmean(pred).float(), torch.nanmean(target).float())

  return loss #mse_loss(torch.mean(pred[:,:5,:4]).float(), torch.mean(target[:,:5,:4]).float())# train function

def custom_transform(image):#add
    im_aug = tfs.Compose([
...
if OPTIMIZE_FLAG or CALC_FI_FLAG or USE_BEST_PARAMS:
  ...
  dict_inp, keys, dict_oco, dates, ti_days_since = get_500m_dataset(xLims=Lims[0], yLims=Lims[1], xLims_test = xLims_test, yLims_test = yLims_test, d_start=d_start_train, d_end=d_end_train, USE_MODEL_OUT=USE_MODEL_OUT, SCALE_FACTOR=SCALE_FACTOR, DATA_OPTION=DATA_OPTION)
...
def train_evaluate(params, validation_split=validation_split, gpu_id=0, prod=False, test=False, FI_EPOCH=False, CALC_TESTLOSS_EPOCH=False):  #
  ...
  df_losses.to_csv(pathPref+results_folder+'/Optimization_Loss.csv')
  if prod:
    return test_loss, model, optimizer, df_losses, pred_mat, targ_mat, dataloader_test, dataloader_tr
  else:
    return test_loss
...
class Objective:
    ...
    def __call__(self, trial):
        params = {'learning_rate': trial.suggest_float('learning_rate', 1e-8, 1e-4),
...
if USE_BEST_PARAMS == False and OPTIMIZE_FLAG == True:
  ...
  if ENQUEUE_TRIAL:
    if DATA_OPTION == 1 and MOD==5:
      study.enqueue_trial({'learning_rate': 9.95018692735643e-05, 'weight_decay': 0.00044042494966214577, 'max_epochs': 19}) #DO12, MOD5
  ...
  test_loss, model, optimizer, df_losses, pred_mat, targ_mat, dataloader_test, dataloader_tr = train_evaluate(best_params, validation_split=validation_split, prod=True, FI_EPOCH=FI_EPOCH, CALC_TESTLOSS_EPOCH=CALC_TESTLOSS_EPOCH)
...
if OPTIMIZE_FLAG or USE_BEST_PARAMS:
  #train
  targ, pred = RunSet(model_woTestSet, dataloader_tr, device='cuda:0')
  X = targ.flatten()
  Y = pred.flatten()
  ...
  ax_histy.set(ylabel='count')
  # Draw the scatter plot and marginals.
  ax.set(xlabel="OCOx target", ylabel="SIF Prediction", )
  low, up = -.25, 1.75
  ax.set_xlim([low, up])
  ax.set_ylim([low, up])
  ax.set_xticks(np.arange(low, up, 0.25))
  ax.set_yticks(np.arange(low, up, 0.25))
  ax.tick_params(axis="x", labelsize=10)
  ax.tick_params(axis="y", labelsize=10)
  scatter_hist(X, Y, ax, ax_histx, ax_histy)
  ax.plot(X, 1*X, color='gray')  
  plt.savefig(pathPref+results_folder+'/TrainGT_SIF_woTestSet.png', format='png', bbox_inches = 'tight', pad_inches = 0.1) #, bbox_inches = 'tight', pad_inches = 0.1
  plt.close()
  ...
  scatter_hist(X, Y, ax, ax_histx, ax_histy)
  ax.plot(X, 1*X, color='gray')
  #plt.title('OCOx footprints to SIF predictions in testing area')
  plt.savefig(pathPref+results_folder+'/GT_SIF_woTestSet.png', format='png', bbox_inches = 'tight', pad_inches = 0.1) #, bbox_inches = 'tight', pad_inches = 0.1
