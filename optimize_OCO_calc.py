...
from ModelClass5_2 import SR_SIF
...
results_folder = 'DO'+str(DATA_OPTION)+'_MOD0'+str(MOD) + '.1_comp2OCO_DLcorrected_train_E-10-30_N40-55_test_E5-15_N45-50_small_calc/'
...
if USE_BEST_PARAMS:
  ...
  #best_params = {'learning_rate': 0.0003414671594997257, 'weight_decay': 9.756837399842312e-05, 'max_epochs': 21} #best par of the new model
  best_params = {'learning_rate': 0.0000341467159499725, 'weight_decay': 9.756837399842312e-05, 'max_epochs': 7} #best par of the new mode
...
def Calc_Loss(pred, target):
  ...
  return mse_loss(torch.mean(pred).float(), torch.mean(target).float()) #mse_loss(torch.mean(pred[:,:5,:4]).float(), torch.mean(target[:,:5,:4]).float())
'''
...
class Objective:
    ...
    def __call__(self, trial):
        params = {'learning_rate': trial.suggest_float('learning_rate', 1e-8, 1e-5),
...
if USE_BEST_PARAMS == False and OPTIMIZE_FLAG == True:
  ...
  if ENQUEUE_TRIAL:
    if DATA_OPTION == 1 and MOD==5:
      study.enqueue_trial({'learning_rate': 0.0000341467159499725, 'weight_decay': 9.756837399842312e-05, 'max_epochs': 7}) #DO12, MOD5
...
if CALC_FI_FLAG:
  FeatureImportance(model_woTestSet, pathPref, results_folder, xLims=np.array([10,15]),yLims=np.array([45,50]), d_start=d_start_test, d_end=d_end_test, USE_MODEL_OUT=False, BATCH_SIZE_TEST=BATCH_SIZE_TEST, FI_RUNS=1, DATA_OPTION=DATA_OPTION, SaveFileName='Results_FeatureImportances.nc')
